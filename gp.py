...
def gp_posterior(X: torch.Tensor, y: torch.Tensor, x_test: torch.Tensor, cov: str = 'se', sigma_y: float = 0.0, l: float = 1.0):
    ...
    K_tt = covariance_matrix(x_test, x_test, cov=cov, l=l) + (1e-8)*torch.eye(x_test.shape[0])  # add sigma_y here for y_* predictive (as opposed to f_*)
...
class GP(nn.Module):
    ...
    def forward(self, x_test: torch.Tensor, X: Optional[torch.Tensor] = None, y: Optional[torch.Tensor] = None, cov: str ='se'):
        if X is None:
            return gp_prior(x_test, cov=cov, l=self.l)
        else:
            assert y is not None
            return gp_posterior(X, y, x_test, cov=cov, l=self.l, sigma_y=self.sigma_y)
